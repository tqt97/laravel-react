...
- **Duplicate queries** — identical SQL _and_ identical bindings. Usually a query that should have been cached or hoisted out of a loop.
...
- The `Dup` column only counts _exact_ duplicates (same bindings). For N+1 read the "repeated query shapes" section instead — that is where a per-record lazy load shows up.
